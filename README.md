...
* **メリット**: 特殊なプラグインを一切必要とせず、標準的なウェブブラウザやビューアーから `http://192.168.4.1/stream` にアクセスするだけで、リアルタイム映像を表示可能です。チャンク形式（`httpd_resp_send_chunk`）による連続送信でネットワークオーバーヘッドを最小化しています。
